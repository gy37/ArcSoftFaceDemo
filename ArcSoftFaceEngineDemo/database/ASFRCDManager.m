...
- (NSArray*)allPersons
{
    ...
    {
        ASFRPerson* person = [[ASFRPerson alloc] initWithCDPerson:cdPerson];
...
- (BOOL)updatePersonFeatureData:(NSArray *)arrayPersons
{
    ...
    for (ASFRPerson *person in arrayPersons) {
...
- (void)deletePerson:(int)personId {
    NSFetchRequest* request = [AFRCDPerson fetchRequest];
    NSPredicate* predicate = [NSPredicate predicateWithFormat:@"personID==%d", personId];
    [request setPredicate:predicate];
    NSManagedObjectContext *managedContext = self.managedObjectContext;
    NSError* error = nil;
    NSArray* objects = [self.managedObjectContext executeFetchRequest:request error:&error];
    ...
    for (AFRCDPerson *cdPerson in objects) {
        [managedContext deleteObject:cdPerson];
    }
    [self saveContext:managedContext];
}
